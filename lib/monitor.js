...
export function log(msg) {
  const args = getDate("log " + msg);
  log_add(args);
};

export function warn(msg) {
  const args = getDate("warn " + msg);
  log_add(args);
};

export function error(msg, exception) {
  const args = getDate("err " + msg);
  log_add(args);
  error_add(args);
  if (config.production) {
    sendError(msg, exception);
  } else {
    debug(msg, exception);
...
export function install(app, options) {
  ...
  app.use(path + "/*", (req, res, next) => {
    ...
    if (!ALLOW_ORIGINS.includes(origin)) {
      origin = (!config.production) ? "*" : "origin-denied";
    }
