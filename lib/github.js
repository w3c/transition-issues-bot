...
class Repository {
  ...
  async getIssues(ttl) {
    const param = {};
    if (this._issues && ttl === undefined) return this._issues;
  ...
  async getOpenIssues() {
    return (await this.getIssues()).filter(issue => issue.state === "open");
  }

  async getClosedIssues() {
    return (await this.getIssues()).filter(issue => issue.state === "closed");
  }
