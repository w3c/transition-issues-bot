...
const NOTIFICATIONS = path.resolve(__dirname, "../notifications.json");
...
const WEBHOOKS = path.resolve(__dirname, "../webhooks.json");
...
async function message(issue) {
  ...
  let status = "(unknown?)";
  if (issue.title.indexOf("] PR Request")) {
    status = "for a new Proposed Recommendation";
  } else if (issue.title.indexOf("] REC Request")) {
    status = "for a Recommendation";
  } else if (issue.title.indexOf("] CR Request")) {
    status = "for a Candidate Recommendation";
  } else if (issue.title.indexOf("] CR Snapshot Update Request")) {
    status = "for an update as a Candidate Recommendation Snapshot";
  } else if (issue.title.indexOf("] FPNote for")) {
    status = "for a First Public Note";
  } else if (issue.title.indexOf("] FPWD Request")) {
    status = "for a First Public Working Draft";
  } else if (issue.title.indexOf("] REC update for")) {
    status = "for an editorial update to a Recommendation";
  } else if (issue.title.indexOf("] Obsolete")) {
    status = "to obsolete a Recommendation";
  } else if (issue.title.indexOf("] Supersede")) {
    status = "to supersede a Recommendation";
  } else
  mail.subject = "[transition] " + issue.title;
  mail.text    = `This is a transition request ${status}`
    + "\n  " + issue.title
