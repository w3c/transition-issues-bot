...
async function message(issue) {
  ...
  if (issue.title.indexOf("] PR Request") !== -1) {
  ...
  } else if (issue.title.indexOf("] FPNote for") !== -1) {
    status = "for a First Public Note";
  } else if (issue.title.indexOf("] FPWD Request") !== -1) {
    status = "for a First Public Working Draft";
  } else if (issue.title.indexOf("] REC update for") !== -1) {
